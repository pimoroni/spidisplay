...
# the clock, while GIFPlayer supplies frames from a GIF decoded at construction and
...
# What picovector.image.load() decodes, an animated GIF being GIFPlayer's job
...
class ImagePlayer:
    """The clock and the traversal over a sequence a subclass supplies, reporting the frame to draw."""
    ...
    def __init__(self, frames, timings, fps=None, loop=True, ping_pong=False, first_as_last=False,
        ...
        if not self.__clocked:
            ...
            # The true interval decides, the rounding below reaching 1ms from well under it
            if fps <= 0 or 1000 / fps < 1:
            ...
            self.__timings = (int(1000 / fps + 0.5),) * self.__frames
    ...
    def __build_order(self):
        ...
        # lap wraps, and a one-shot is 2n-1, closing on frame 0.
    ...
    def __origin(self, step):
        ...
        # earned by arriving and not granted to a player set down there
        if not self.__clocked:
            return 0
    ...
    @property
    def frame(self):
        """The frame number to play, a ping-pong reporting the same number out and back."""
    ...
    def is_reversed(self):
        """Whether the frame number is decreasing, either from reverse() or on a ping-pong's way back."""
        if self.__ping_pong:
    ...
    def measured_ms(self):
        """The last interval between frames reaching the caller, changing where has_advanced() is True."""
...
class GIFPlayer(ImagePlayer):
    ...
    def __init__(self, path, fps=None, loop=True, ping_pong=False, first_as_last=False, hold=0,
                 paused=False):
        """Construct a player over the GIF at path, holding every frame it decodes."""
        # Diagnostics only, one call being no wait worth announcing
...
class SequencePlayer(ImagePlayer):
    ...
    def __init__(self, folder, fps=None, timings=None, loop=True, ping_pong=False,
        ...
        """Construct a player over a folder of images, holding every frame it loads.

        timings names one delay in milliseconds a frame, where the file names declare none.
        """
        ...
        # Keys built once, sort(key=...) recomputing a key on every comparison here
    ...
    def __numbers_in(name):
        # So frame_10 sorts after frame_9, lexicographic order being silently wrong past nine
        found = []
        digits = ""
    ...
    def __load(folder, names, paths):
        # Blocks for seconds, so it reports as it goes, dots at LOG_INFO and a line a
        # frame at LOG_DEBUG. No estimate, the cost a frame following the image size.
