...
#
# Checks playback.py against synthetic frame counts, and against whatever animations
# the settings below name. No asset ships with this, so those phases are skipped until
# a checkout provides its own and says what each one holds.
#
# The phases are the traversal, printed as a step, frame and delay table and asserted
# for the two orders that are easy to get wrong. Then the origin rule, a dwell being
# earned by arriving. Then placement, pause and reverse, including a reverse part way
# through a dwell. Then every raise, each naming its fix. Last a run on a real pair,
# reporting what was achieved against what was asked.
#
...
# The animations on the board, each named with what it holds so the figures can be
# checked. An empty setting skips the phase that reads it.
GIFS = ()               # Each a path, its frame count and the delay every frame declares
NAMED_FOLDER = ()       # A path, frame count and delay, the delay coming from the names
BIG_FOLDER = ()         # The same, for frames larger than the panel
PLAIN_FOLDER = ()       # A path and frame count, the names carrying no delay

PAIR = True             # The pair phase also needs two entries in GIFS
ROTATION = 90
PAIR_MS = 10_000
...
def show_cycle(player, label):
    """Print the cycle with step, frame and delay lined up."""
...
def check_traversal():
    ...
    for count in (1, 2, 3, 8):
        ...
        # A one-shot must come to rest back home, or a play in and out stops a frame short of retracted
...
def check_origin():
    """Check that a dwell is earned by arriving, not by a placement on the step."""
...
def check_position_and_pause():
    ...
    # A frame number reads back, so one player can feed two screens a fixed distance
    # apart, where a second player would cost the heap twice
    player = numbered(6, ping_pong=True)
    player.to_frame(4)
...
def check_gifs():
    """The GIFs a checkout named, and what their figures come out as."""
    print("The GIFs on the board")
    if not GIFS:
        print("  skipped, GIFS names none")
        return

    for path, frames, delay in GIFS:
        ...
        image = player.image
        assert player.image_at(1).width == image.width, "image_at reaches a frame the player is not on"
...
def check_sequence():
    """Check a folder of frames, its order, where its delays come from, and what it costs."""
    from playback import SequencePlayer

    print("Folders of frames")
    if not (NAMED_FOLDER or BIG_FOLDER or PLAIN_FOLDER):
        print("  skipped, no folder is named")
        return

    for folder, count, delay in (named for named in (NAMED_FOLDER, BIG_FOLDER) if named):
        t0 = time.ticks_ms()
    ...
    if PLAIN_FOLDER:
        # Ordered by the numbers in the names, not lexicographically, or any export past
        # nine frames plays out of order and says nothing about it.
        folder, count = PLAIN_FOLDER
        t0 = time.ticks_ms()
        player = SequencePlayer(folder, fps=25)
        took = time.ticks_diff(time.ticks_ms(), t0)
        ...
        assert player.frames == count
        assert player.cycle_ms == count * 40
        print("  {:<12} {} frames at 25fps, in numeric order, loaded in {}ms".format(
            folder, player.frames, took))
    ...
    if NAMED_FOLDER:
        ...
        folder, count, _ = NAMED_FOLDER
        player = SequencePlayer(folder, timings=(50,) * count)
        assert player.cycle_ms == 50 * count
        print("  timings given by hand override what the names declare")

        # A folder's frames are reachable by number too, where GIFPlayer has sheet and
        # this has only the file names. One player can then feed two screens a fixed
        # distance apart.
        ...
        refuses("timings of the wrong length",
                lambda: SequencePlayer(folder, timings=(50, 50)))

    if PLAIN_FOLDER:
        refuses("a folder whose names carry no delay",
                lambda: SequencePlayer(PLAIN_FOLDER[0]))

    # The root holds no numbered frames, whatever else a board carries
    refuses("a folder with no images in it", lambda: SequencePlayer("/"))
...
def check_pair():
    ...
    try:
        # ScreenPair announces its own calibration, so nothing is printed here
        pair = ScreenPair(Screen280(port_a, reserve=Reserve.FULL_SIZE_IMAGES),
                          Screen280(port_b, reserve=Reserve.FULL_SIZE_IMAGES))

        slow = GIFPlayer(GIFS[0][0])
        fast = GIFPlayer(GIFS[1][0])
        moved = [0, 0]
        pushes = 0
        t0 = time.ticks_ms()
...
check_logging()
check_sequence()
if PAIR and len(GIFS) > 1:
    check_pair()
else:
    print("\nOn a pair\n  skipped, the pair phase needs PAIR set and two entries in GIFS")
