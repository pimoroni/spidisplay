...
#
# Checks that a ScreenPair holds the two panels' skew at its predicted floor, and that
# pausing and resuming shows no visible stagger.
#
# Six phases run.
# Placement resolution against the documented table.
# An aligned run reporting steady skew.
# A cadence run, where normal animation must never reach the resync trigger.
# A paced run at a fixed interval near the pair's own frame time.
# Pause-and-resume trials, with eyes on the resume instant.
# Solo updates on each screen, which must hand back the follower's panel state.
#
# Set SCREEN to the panel type on the ports. Every phase runs the default reserve and
# an SRAM canvas redrawn per frame, which the thresholds were measured against. Keep
# eyes on both panels.
...
SCREEN = Screen154          # Or Screen280
ALIGN_MS = 30_000           # The aligned run, raised for an acceptance soak
CADENCE_FRAMES = 200

# A slot a pair frame can only just fill, and the interval an animation is most likely
# to be authored at
PACE_MS = 80
PACE_FRAMES = 120
...
# An update this late must have spent a resync, sitting above the worst normal update
# on either panel type and below the cheapest resync frame, which carries the capture
# and excursion on top of the frame
RESYNC_MIN_MS = 120
GRID_PITCH = 20
...
def check_resolution():
    """Check every row of the documented placement table, and the shapes that must reject."""
    ...
    assert offsets((5, None)) == ((5, None), (5, None))     # Shared, 5 being no (x, y)
...
def paced_run():
    """Drive at an animation's fixed interval, where a late frame loses its TE edge.

    An animation asks for a frame every PACE_MS, and one overrunning that slot leaves
    the next already due, which begins it without waiting. Only te_timeouts reports that.
    """
    ...
    print("  median {}ms, p95 {}ms, worst {}ms, {} of {} overran the slot, "
          "te_timeouts {}".format(
              ...
              over, len(spans), late))
    # Overrunning is allowed, a caller driving from a clock skipping instead of falling
    # behind. Losing the edge is not.
...
def solo_updates():
    ...
    assert held_back == default_back + pair.__pad_lines, (
        f"the pair resumed at a back porch of {held_back} lines against the "
        f"{default_back} + {pair.__pad_lines} its calibration chose")
    report_skews("resumed", skews, timeouts0)
    print()
...
built = (SCREEN(port_a), SCREEN(port_b))
pair = None
...
try:
    ...
    # The pair's first screen, whose settings the banner reports
    ...
    if not pair.is_aligned():
        ...
        print("calibrated in {}ms, predicted floor {:.0f}us, pad {} porch lines".format(
            calibration_ms, pair.__floor_us, pair.__pad_lines))
