...
#
# Checks the frame converter, both the sources it must accept and refuse, and that an
...
#
# Needs a 2.8" on SP/CE A. Each asset path below reports N/A when absent.
...
# The assets on the board, each left empty so no case names a file that is not here.
# An empty setting skips the case that reads it.
GIF_PATH = ""           # A panel-sized strip of solid CELL_COLORS frames
GIF_WIDE_PATH = ""      # Frames narrower than the panel, so the strip runs wider than it
INDEXED_PATH = ""       # An indexed PNG with transparent, translucent and opaque entries
...
BAR_W = 20              # A bar's width in pixels
PHASE_STEP = 10         # A cell's bar phase in pixels, so no two cells draw the same bars
...
# Case names to skip, for stepping past one that locks a build missing the check
SKIP = ()

# The exceptions picovector and update() decline a case with, each reported as N/A
...
def bar_row(width, index):
    # One packed row of the cell at index, vertical bars of its colour on black
...
def stride_of(view):
    # Whether an image built over the view's own memoryview samples the same pixels,
    # so the view is contiguous. None where the view cannot be modelled.
    try:
...
# Phase 1, the sources update() must accept and refuse.
...
def phase_sources():
    ...
    # Whether the reported length tracks the buffer, a nominal one comparing a number with itself
    try:
    ...
    if not GIF_PATH:
        verdict("N/A", "GIF_PATH names no file")
    else:
        try:
...
def phase_readback():
    ...
    # Check if this build has the RGB565 packer the expectations below need
    if 16 not in spidisplay.BITDEPTHS:
        verdict("N/A", "this build converts to 12-bit only, and the byte expectations are 16-bit")
        return
    # Drop the low claims, so the region below reaches up to the display's own
    ...
    try:
        ...
        # Half the panel, so the ring's two slots hold a whole frame
        screen = SCREEN(port, bitdepth=16, band_lines=SCREEN.HEIGHT // 2, cache_columns=0)
        screen.brightness(1.0)
        W, H = screen.width, screen.height
        ROW_BYTES = W * 2
        FRAME_BYTES = ROW_BYTES * H
        # What is left free is where the display's claim begins, so this is the ring's offset
        ...
        # The ring must hold a whole frame in row order, or nothing below means anything
        ...
        # Palettised strips, every frame one solid colour, so the expected bytes need
        # no reference image
        ...
        # Only an indexed source composites, ahead of the pixel loop, so an RGBA
        # source's alpha is ignored. driver/pixel_formats.hpp has the measurement.
        ...
        if not INDEXED_PATH:
            verdict("N/A", "INDEXED_PATH names no file")
            return
