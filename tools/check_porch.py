...
#
# Checks that a panel honours a porch change at runtime, latches it at a frame boundary,
# and shows nothing on the glass doing it. A group's alignment trims and dithers a
# member's rate in porch lines, so a new panel has to hold all three.
#
# Three experiments run on the panel UNDER_TEST names.
# 1, sweep, the achieved period and TE pulse at each porch, against one line time a line.
# 2, latch, consecutive TE periods across a write, including writes inside a long
#    blanking asking for a much shorter one, where a spanning period matching neither
#    steady value is the fault.
# 3, glitch, eyes on the panel, the porch alternated under a still image and a v_sync
#    stream, by one line and by the wider step GLITCH_STEPS holds for that panel type.
#
...
# default porch is written back whatever happens. Rows under PULSE_FPS are named,
# a 1.54 pulsing there on its own account.
#
# Experiment 3 needs eyes on the panel under test.
...
# The harness as it is wired, the port's own CS first
HARNESS = (
...
UNDER_TEST = 27             # Run both panel types, the 1.54 being the thin case for tear margin
BAUDRATE = 24_000_000

TEM_VBLANK = b"\x00"
PORCH_TAIL = st7789.PORCH_TAIL
DEFAULT_PORCH = st7789.PORCH
...
PORCHES = (
    ...
    (20, 20),
    (28, 28),
    (6, 6),     # Below the default, where a trim speeds a panel up
    (2, 2),
...
PROBE_MS = 500
SETTLE_MS = 100
LATCH_PAIR = ((12, 12), (28, 28))       # A 32-line step, an excursion of 8 and the 24 a walk reaches
BLANK_PAIR = ((28, 28), (6, 6))         # A 44-line shortening, wide enough to cut a blanking in force
BLANK_PLACES = (0.05, 0.45, 0.85)       # Of the blanking in force, past the TE rise
LATCH_EDGES = 12            # Falls a latch run takes, the write landing halfway
LATCH_RUNS = 4
...
PULSE_FPS = 50              # The 1.54 pulses below about this, which the sweep's longer porches reach
GRID_PITCH = 20
...
# PSRAM image still draws, costing about twice as much per pixel to convert
try:
    canvas = panel.canvas()
except ValueError:
...
def capture_falls(count, timeout_ms=2000, on_fall=None):
    ...
    a write takes DC back as an output. It costs no edge, following a fall that has
    just happened, so the next one is a whole period away.
    """
    read_te()
    falls = []
...
def line_time(rows):
    """Microseconds a scan line costs, least squares over the sweep's slot and period readings."""
    count = len(rows)
...
def latch(pair, steady, place, runs, s_line):
    ...
    None writes just after a fall, at the start of the visible scan. A fraction waits
    out the visible scan and then that much of what the blanking leaves once the
    write's own cost is taken off, so the write completes before the blanking ends.
    """
    placed = []
    half = LATCH_EDGES // 2
...
def write_frame(index):
    """Write one v_sync frame, so the porch is the only thing that can disturb the glass.

    The panel is built te=False, a shared DC line permitting nothing else, so this goes
    through the display to reach v_sync. Unsynced, a write longer than the refresh tears
    every time and would read as a porch fault.
    """
    draw(BACKGROUNDS[index % 2])
    dc.value(0)                     # Pre-discharge, as arm() does for every armed frame
