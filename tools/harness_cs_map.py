...
#
# Maps a hub or harness, saying which panel sits on which chip select and which type
# each one is. Every candidate CS is painted its own colour with its own count of black
# bars and left lit, so the whole harness reads off the glass in one look.
#
# Run once as Screen280 to find which lines are populated, where a 2.8" fills and a
# 1.54" takes the top 240 rows and drops the rest. Run again as Screen154 to sort the
...
from ports import ScreenPort, gpio_number
from spce import SPCE_A_PINS

EXTRA_CS = (24, 25, 26, 27, 37)         # Every CS beyond the port's own, an empty line staying dark
SCREEN_CLASS = screens.Screen280        # The geometry this pass paints, Screen154 being the other
BAUDRATE = 24_000_000
BAR_PITCH = 26
...
for candidate in (port.cs,) + EXTRA_CS:
...
panels += [SCREEN_CLASS(port, cs=Pin(n), dc=port.dc, te=False, baudrate=BAUDRATE)
           for n in EXTRA_CS]
labels = (gpio_number(port.cs),) + EXTRA_CS
panels[0].brightness(1.0)
# Many band claims can leave no room for an SRAM canvas, and nothing here is timed
try:
