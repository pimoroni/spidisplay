...
#
# Checks that a Screen and the SPIDisplay under it agree on the panel, dimensions, rate
# and banding all being held twice, once in each.
#
# A keyword override has to reach the C constructor to mean anything. The second screen
# here is overridden, size() says the number arrived, and a frame stopping short of the
...
# What is wired. Import Screen154 as well to put a 1.54 on either port.
SCREEN_A = Screen280
SCREEN_B = Screen280                                # None for a single panel
OVERRIDES_B = {"height": 280, "framerate": 45}      # Built on top of the class's own

# Requested rows per DMA band, which band_rows() reports back after clamping to the
# panel height. Each screen's rows claim real SRAM for as long as it lives.
BAND_LINES = 16
...
def report(label, screen):
    ...
    # The workspace claim is a band pair plus the cache, each rounded to 4 bytes.
    ...
    # The divider only reaches even fractions of clk_peri, so the rate rounds down.
    # Equal is what a reachable rate gives, and above the request would be a fault.
...
def draw_ruler(screen, label):
    ...
    A plain image(), not canvas(), nothing here being timed, so the SRAM region is not
    worth spending and two screens taking canvas() would share it.
    """
...
port_b = ScreenPort(SPCE_B_PINS) if SCREEN_B else None

screens = [(SCREEN_A(port_a, band_lines=BAND_LINES), "A, class defaults")]
if SCREEN_B:
    screens.append((SCREEN_B(port_b, band_lines=BAND_LINES, **OVERRIDES_B),
...
try:
    ...
    ports.shutdown(*(port for port in (port_a, port_b) if port is not None))
