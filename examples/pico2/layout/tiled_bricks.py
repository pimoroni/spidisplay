...
That tile is a fiftieth of what a wall covering the panel would cost as a full-size image, so
tiling is the difference between an asset that fits anywhere and one that has to be budgeted
for. On a host with no PSRAM that matters twice over, the panel-sized image being most of the
heap.
...
# The Display Pack is a port already, its pins built in, and the screen is its panel
port = PicoDisplay28()
screen = ScreenType(port)

# Two courses of one brick, and the driver makes a wall of it
wall = image(BRICK_W, BRICK_H * 2)
print(f"a {BRICK_W}x{BRICK_H * 2} wall tile, "
      f"{BRICK_W * BRICK_H * 2 * spidisplay.PIXEL_BYTES // 1024}KB,"
