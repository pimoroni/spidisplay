...
# quarter onto the panel. Half the panel's size, since a matrix and its mask are two images and a host
# with no PSRAM has not the heap for two full-size ones. Every update doubles it back up, so a lamp
# lands on a 2 by 2 block and the matrix is half as fine as PIXEL alone would make it
canvas = image(screen.height // 2, screen.width // 2)
...
try:
    while not rp2.bootsel_button():
        draw(travelled)
        screen.update(canvas, rotation=90, pixel_double=True)
