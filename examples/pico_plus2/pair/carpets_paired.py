...
# Which screens are on the ports, "2.8" or "1.54".
# A pair wants two of the same size, each panel holding its own picture
SCREEN_SIZE = "2.8"
ScreenType = SCREEN_TYPES[SCREEN_SIZE]

# The Display Pack is a port already, and a second screen chains from the SP/CE connector it
# brings out. Not the host's own connector, whose clock pin puts it on the same SPI peripheral
# as the pack, and one peripheral is one stream
pack = PicoDisplay28()
...
pair = ScreenPair(ScreenType(pack, reserve=Reserve.FULL_SIZE_IMAGES),
                  ScreenType(chained, reserve=Reserve.FULL_SIZE_IMAGES))
...
print(f"a {HEX_W}x{HEX_H} carpet and a {BLOCK_TILE_W}x{BLOCK_TILE_H} one,"
      f" {(HEX_W * HEX_H + BLOCK_TILE_W * BLOCK_TILE_H) * spidisplay.PIXEL_BYTES // 1024}KB"
      f" for two panels")
...
pixel = spidisplay.PIXEL_BYTES
for row in range(HEX_H):
    line = row * stride
    for column in range(HEX_W // 2):
        near = line + column * pixel
        far = line + (HEX_W - 1 - column) * pixel
        raw[far:far + pixel] = raw[near:near + pixel]
