...
This file holds what a maintainer would otherwise re-derive from the code. The MicroPython side that
drives the module, the `screens` package and `st7789.py`, ships with the board firmware today and
follows into this repository, along with the customer reference for it.

## Files

`driver/` is the half that references no MicroPython API, which is why `tests/` can build it on a
host. `bindings/` is the half that does.
...
| `driver/scanline.hpp` | the conversion kernels: RGBA8888 or palettised source to RGB444 or RGB565 rows, with rotation, mirror, pixel doubling and tiling |
| `driver/descriptor.hpp` | the descriptor a kernel walks, built once per frame from the source, the placement and the rotation |
| `driver/pixel_formats.hpp` | one packer per destination format, each owning its format's arithmetic |
...
- `te_short_waits()`: frames whose wait ended on a pulse it watched rise and that fell inside
  ...
  and `te_probe()`'s period is what names that fault. It has no known live case on the panels this
  driver ships with, and is kept for the hosts and panels it has not met.
